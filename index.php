...
$app->post('/log_in', function(){
    ...
    try {
        ...
        if($username==$username_db && $password==$password_db){
		        ...
		        if($status_login=='1'){
		        	$sth2 = $db->prepare("UPDATE user SET status_login = '1' WHERE username=:username AND password=:password");
		            $sth2->bindParam(':username', $username, PDO::PARAM_INT);
		            $sth2->bindParam(':password', $password, PDO::PARAM_INT);
		            $sth2->execute();
		        	}
		        	$output =  array('status' =>  "0",
		        					 'pesan' => "is already logged in"
		        					 );
...
$app->post('/logout', function(){
    ...
    try {
        ...
        if($status_login=='1'){
		            ...
		            $sth3->bindParam(':username', $username, PDO::PARAM_INT);
		            $sth3->execute();
            }
            $output = array(
                    'status' => "0",
...
$app->post('/register', function() {
    ...
    try {
        ...
        if ($row['count'] > 0) {
            $output = array(
                'status' => "0",
                'operation' => "username is already exist"
            );
            echo json_encode($output);
            $db = null;
            return;
        } else {
				$sth = $db->prepare("INSERT INTO user (nama,username,password) 
